( function ( blocks, blockEditor, components, element, i18n, apiFetch, serverSideRender ) {
	...
	function normalizeLabels( labels ) {
		return Object.assign(
			{
				zencoins: 'ZENCOINS:',
				usageText: 'For all Courses and Fire & Ice',
				validityText: 'Valid for 3 Months beginning with the date of purchase',
				largeValidityText: 'Valid for 6 Months beginning with the date of purchase',
				button: 'Book now'
			},
			labels || {}
		);
	...
	function Edit( props ) {
		...
		var blockProps = useBlockProps( { className: 'zpb-packages-editor' } );
		...
		useEffect( function () {
			apiFetch( { path: '/zen-purchase-blocks/v1/package-products' } )
				.then( function ( response ) {
					setProducts( response || [] );
					setLoading( false );
				} )
				.catch( function () {
					setError( __( 'Could not load Zencoin package products.', 'zen-purchase-blocks' ) );
					setLoading( false );
				} );
		}, [] );
		...
		return el(
			...
			el(
				...
				el(
					PanelBody,
					{ title: __( 'Package Products', 'zen-purchase-blocks' ), initialOpen: true },
					loading ? el( Spinner ) : null,
					error ? el( Notice, { status: 'error', isDismissible: false }, error ) : null,
					! loading && products.length === 0
						? el( 'p', null, __( 'No products with Zencoin product type Package were found.', 'zen-purchase-blocks' ) )
						: null,
			...
			el(
				...
				el( ServerSideRender, {
					block: 'zen-purchase-blocks/zencoin-packages',
	...
	registerBlockType( 'zen-purchase-blocks/zencoin-packages', {
		edit: Edit,
		save: function () {
			return null;
		}
